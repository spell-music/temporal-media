...
instance (Num t, IfB t, OrdB t) => Semigroup (Track t a) where
  x <> y          = x `mappendTrack` y
...
instance (Num t, IfB t, OrdB t) => Monoid (Track t a) where
    mempty = Track 0 mempty
    mappend = mappendTrack

#endif

mappendTrack :: (Num t, IfB t, OrdB t) => Track t a -> Track t a -> Track t a
mappendTrack (Track d es) (Track d' es') =
...
#if MIN_VERSION_base(4,11,0)
instance Semigroup (TList t a) where
    (<>) = mappendTList
...
instance Monoid (TList t a) where
    mempty  = Empty
    mappend = mappendTList

#endif

mappendTList :: TList t a -> TList t a -> TList t a
mappendTList Empty   a       = a
mappendTList a       Empty   = a
mappendTList a       b       = Append a b
